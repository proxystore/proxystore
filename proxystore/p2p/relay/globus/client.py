"""Client interface to a [`GlobusAuthRelayServer`][proxystore.p2p.relay.globus.server.GlobusAuthRelayServer]."""  # noqa: E501
...
class GlobusAuthRelayClient:
    """Client interface to a [`GlobusAuthRelayServer`][proxystore.p2p.relay.globus.server.GlobusAuthRelayServer].
    ...
    Tip:
        ...
        ```python
        from proxystore.p2p.relay import GlobusAuthRelayClient

        async with GlobusAuthRelayClient(...) as client:
            await client.send(...)
    ...
    Note:
        ...
        [`connect()`][proxystore.p2p.relay.globus.client.GlobusAuthRelayClient.connect]
        is called. Initializing the client with `await` will call
        [`connect()`][proxystore.p2p.relay.globus.client.GlobusAuthRelayClient.connect].
        ```python
        client = await GlobusAuthRelayClient(...)
        ```

    Args:
        address: Address of the relay server. Should start with `ws://` or
            `wss://`.
        authorizer: Globus authorizer used to generate authorization headers.
        ...
        verify_certificate: Verify the relay server's SSL certificate. This
    ...
    Raises:
        ...
        ValueError: If address does not start with `wss://`.
    """  # noqa: E501

    def __init__(
        self,
        address: str,
        authorizer: globus_sdk.RenewingAuthorizer,
        *,
        ...
        if not address.startswith('wss://'):
            raise ValueError(
                'Relay server address must start with wss://.'
                f'Got {address}.',
            )
