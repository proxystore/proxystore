...
from proxystore.globus.app import _get_app_name
...
def get_native_app_auth_client(
    ...
    return globus_sdk.NativeAppAuthClient(
        client_id=client_id,
        app_name=_get_app_name() if app_name is None else app_name,
    )
...
def get_transfer_client(
    ...
    Args:
        globus_app: [`GlobusApp`][globus_sdk.experimental.globus_app.GlobusApp]
            used to initialize the transfer client. If `None`, a default
            [`UserApp`][globus_sdk.experimental.globus_app.UserApp] is created
            using [`get_user_app()`][proxystore.globus.app.get_user_app].
