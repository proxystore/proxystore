...
class UCXStore(Store[UCXStoreKey]):
    ...
    def __init__(self, name: str, *, interface: str, port: int, cache_size: int = 16, stats: bool = False, ) -> None:
        """Initialization of a UCX client to issue RPCs to the UCX server.
        This client will initialize a local UCX server (Peer service) that it will
        store data to.
        ...
        super().__init__(name, cache_size=cache_size, stats=stats, kwargs= {"interface": interface, "port": self.port})

    def _start_server(self):
        """Launch the local UCX server (Peer) process using self.host and self.port.
    ...
    def exists(self, key: UCXStoreKey) -> bool:
        ...
        event = pickle.dumps({"key": key.ucx_key, "data": None, "op": "exists"})
    ...
    def set_bytes(self, key: UCXStoreKey, data: bytes) -> bool:
        self._logger.debug(
            "Client issuing set request on key %s with addr %s", key, self.addr
        )

        event = pickle.dumps({"key": key.ucx_key, "data": data, "op": "set"})

        return bool(asyncio.run(self.handler(event, self.addr)))
