...
async def test_connect_twice(relay_server) -> None:
    uuid, _, websocket = await connect(relay_server.address)
...
async def test_connect_reconnect_new_socket(relay_server) -> None:
    uuid1, name, websocket1 = await connect(relay_server.address)
...
async def test_expected_client_disconnect(relay_server) -> None:
    uuid, _, websocket = await connect(relay_server.address)
    client = relay_server.relay_server._uuid_to_client[uuid]
    assert client in relay_server.relay_server._websocket_to_client.values()
    ...
    assert uuid not in relay_server.relay_server._uuid_to_client
    assert (
        client not in relay_server.relay_server._websocket_to_client.values()
    )
...
async def test_server_deserialization_fails_silently(relay_server) -> None:
    _, _, websocket = await connect(relay_server.address)
...
async def test_p2p_message_passing(relay_server) -> None:
    peer1_uuid, peer1_name, peer1 = await connect(relay_server.address)
    peer2_uuid, peer2_name, peer2 = await connect(relay_server.address)
